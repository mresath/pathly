...
export default function Create() {
  ...
  const [name, setName] = useState(t("newAct"));
  const [description, setDescription] = useState("");
  const [icon, setIcon] = useState<ActIcon>("Activity");
  const [stats, setStats] = useState<ToImp[]>([]);
  const [type, setType] = useState<ActivityType>("positive");
  const [diffValue, setDiffValue] = useState<string>("3");
  ...
  const handleSubmit = () => {
    const id = idFromName(name, Object.keys(activities));
    ...
    router.dismiss();
    toast.success(t("activityCreated").replace("{{activity}}", name), { position: ToastPosition.BOTTOM });
  }
