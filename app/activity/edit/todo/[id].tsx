...
export default function Create() {
  ...
  const [activityId, setActivityId] = useState<string>(todo.activityId || Object.keys(activities)[0] || "");
  const activity = useMemo(() => activities[activityId], [activityId, activities]);

  const [name, setName] = useState(todo.name || activity?.name || t("newAct"));
  const [description, setDescription] = useState(todo.description || activity?.description || "");
  const [icon, setIcon] = useState<ActIcon>(todo.icon || activity?.icon || "Activity");
  const [neglection, setNeglection] = useState<boolean>(todo.neglection || true);
  const [date, setDate] = useState(todo.due || new Date(new Date(new Date().setMinutes(Math.round(new Date().getMinutes() / 5) * 5, 0) + 24 * 60 * 60 * 1000)));
  ...
  const handleSubmit = () => {
    ...
    setTodo(id, newTodo);
    router.dismiss();
    toast.success(t("todoEdited").replace("{{todo}}", name), { position: ToastPosition.BOTTOM });
  }
