...
import { ActIcon, useHabit, ActivityIcon, Habit } from '~/lib/habit';
...
export default function Create() {
  ...
  const [activityId, setActivityId] = useState<string>(habit.activityId || Object.keys(activities)[0] || "");
  const activity = useMemo(() => activities[activityId], [activityId, activities]);

  const [name, setName] = useState(habit.name || activity?.name || t("newAct"));
  const [description, setDescription] = useState(habit.description || activity?.description || "");
  const [icon, setIcon] = useState<ActIcon>(habit.icon || activity?.icon || "Activity");
  const [neglection, setNeglection] = useState<boolean>(habit.neglection || true);
  ...
  const handleSubmit = () => {
    ...
    setHabit(id, newHabit);
    router.dismiss();
    toast.success(t("habitEdited").replace("{{habit}}", name), { position: ToastPosition.BOTTOM });
  }
